...
const GameProvider = ({ children }) => {
  ...
  const [host, setHost] = useLocalStorageState('host', {
    ssr: true,
    defaultValue: '',
  })
  const [rounds, setRounds] = useLocalStorageState('rounds', {
    ssr: true,
    defaultValue: '',
  })
  ...
  const leaveGame = async () => {
    ...
    try {
      ...
      const { data: playersInGame, error } = await supabase
        .from('games')
        .select('players_in_game')
        .match({ in_progress: true, pin })

      const playersArray = JSON.parse(playersInGame[0].players_in_game)

      // Find current player & remove from array of players
      const removePlayerFromArray = () =>
        playersArray.filter((player) => player.id !== user.id)

      // Push new array back into db
      const updatePlayers = await supabase
        .from('games')
        .update({
          players_in_game: JSON.stringify(removePlayerFromArray()),
        })
        .match({ in_progress: true, pin })
